...
SELECT p.dni, p.nombre, p.apellidoPaterno, p.apellidoMaterno, p.direccion, 
p.telefono, p.correo_electronico, p.password, 
case
    when p.genero = 'M' then "Masculino"
    when p.genero = 'F' then "Femenino"
    when p.genero = 'O' then 'Otro'
 end as genero,
 p.foto,p.usuario, 
 DATE_format(p.fecha_nac, '%Y-%m-%d') as fecha, pa.des_pais, r.des_region, 
 d.id_distrito as codigodistrito, d.des_distrito as distrito, 
 rs.link_wsp as wsp,rs.link_fb as fb,rs.link_twt as twt,rs.link_ig as ig,rs.link_wtp as wtp FROM persona p 
 ...
 INNER JOIN UsuarioRedSocial rs ON p.dni = rs.dni 
 WHERE p.dni = ??????